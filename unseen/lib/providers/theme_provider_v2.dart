...
enum AppThemeType {
  ...
  privacy, // Privacy-focused theme (Deep Indigo)
  darkPrivacy, // Темная версия Privacy темы
}
...
class ThemeProviderV2 extends ChangeNotifier {
  ...
  AppThemeType _currentTheme = AppThemeType.privacy;
  AppThemeType _lastLightTheme = AppThemeType.privacy;
  ...
  AppColors _createColorsForTheme(AppThemeType theme) {
    switch (theme) {
      ...
      case AppThemeType.privacy:
        return PrivacyColors();
      case AppThemeType.darkPrivacy:
        return DarkPrivacyColors();
    }
  }
  ...
  Future<void> _loadTheme() async {
    try {
      ...
      // В случае ошибки используем Privacy тему
      debugPrint('=== THEME PROVIDER: Error loading theme: $e ===');
      _currentTheme = AppThemeType.privacy;
  ...
  /// Установить Privacy тему
  Future<void> setPrivacyTheme() async {
    await setTheme(AppThemeType.privacy);
  }

  /// Установить темную Privacy тему
  Future<void> setDarkPrivacyTheme() async {
    await setTheme(AppThemeType.darkPrivacy);
  }
  ...
  AppThemeType _getDarkVariant(AppThemeType lightTheme) {
    switch (lightTheme) {
      ...
      case AppThemeType.privacy:
        return AppThemeType.darkPrivacy;
      default:
  ...
  IconData getThemeIcon(AppThemeType theme) {
    switch (theme) {
      ...
      case AppThemeType.privacy:
        return Icons.shield; // Privacy shield icon
      case AppThemeType.darkPrivacy:
        return Icons.dark_mode; // Или Icons.shield (с dark оттенком)
    }
  }
