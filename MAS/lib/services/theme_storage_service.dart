...
class ThemeStorageService {
  ...
  Future<List<CustomThemeData>> loadCustomThemes() async {
    try {
      ...
      debugPrint('=== THEME STORAGE: Loaded ${themes.length} custom themes ===');
      return themes;
    } catch (e) {
  ...
  Future<bool> saveCustomTheme(CustomThemeData theme) async {
    try {
      ...
      if (existingIndex == -1 && themes.length >= _maxThemes) {
        debugPrint('=== THEME STORAGE: Max themes limit reached ($_maxThemes) ===');
        return false;
      }
  ...
  Future<bool> deleteCustomTheme(String id) async {
    try {
      ...
      if (themes.isEmpty) {
        ...
        final jsonString = jsonEncode(jsonList);
        await prefs.setString(_storageKey, jsonString);
        debugPrint('=== THEME STORAGE: Deleted theme, ${themes.length} remaining ===');
      }
