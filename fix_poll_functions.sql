-- Fix device_id type from UUID to TEXT in all poll functions

-- 1. Fix get_poll_data function
CREATE OR REPLACE FUNCTION public.get_poll_data(p_device_id text, p_language_code text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
-- 2. Fix vote_for_option function
CREATE OR REPLACE FUNCTION public.vote_for_option(p_device_id text, p_option_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  INSERT INTO poll_votes (device_id, option_id, voted_at, updated_at)
...
-- 3. Fix unvote_for_option function
CREATE OR REPLACE FUNCTION public.unvote_for_option(p_device_id text, p_option_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
